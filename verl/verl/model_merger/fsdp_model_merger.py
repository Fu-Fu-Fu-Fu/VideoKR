...
class FSDPModelMerger(BaseModelMerger):
    ...
    def _get_device_mesh_array(self, device_mesh) -> np.ndarray:
        try:
            mesh = device_mesh.mesh
        except AttributeError:
            # Checkpoints saved by older PyTorch versions may unpickle a DeviceMesh
            # without the newer _layout field required by the mesh property.
            mesh = getattr(device_mesh, "_mesh", None)
            if mesh is None:
                mesh = getattr(device_mesh, "__dict__", {}).get("mesh", None)
            if mesh is None:
                raise
        ...
        if isinstance(mesh, torch.Tensor):
            ...
            mesh = np.array(mesh, dtype=np.int64)
        return mesh

    def _get_mesh_dim_names(self, device_mesh, mesh: np.ndarray) -> tuple[str, ...]:
        mesh_dim_names = getattr(device_mesh, "mesh_dim_names", None)
        if mesh_dim_names is None:
            mesh_dim_names = getattr(device_mesh, "__dict__", {}).get("mesh_dim_names", None)
        if mesh_dim_names is None:
            mesh_dim_names = ("fsdp",) if mesh.ndim == 1 else ("ddp", "fsdp")
        return tuple(mesh_dim_names)
    ...
    def _extract_device_mesh_info(self, state_dict: dict, world_size: int) -> tuple[np.ndarray, tuple[str, ...]]:
        ...
        if isinstance(weight, DTensor):
            ...
            mesh = self._get_device_mesh_array(device_mesh)
            mesh_dim_names = self._get_mesh_dim_names(device_mesh, mesh)
        else:
            # for non-DTensor
